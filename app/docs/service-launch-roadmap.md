...
**Rating Input을 드래그 슬라이더로, 0.1 단위로 (2026-08-09)** — 감사에서 "기록 1건에 12~13탭, 그중 별점 4.0에만 8탭"으로 지목된 최대 병목을 해결했다.

- **입력**: `-`/`+` 스테퍼와 별 5개 글리프를 제거하고 **드래그 슬라이더 + 숫자** 하나로 교체. 0.5~5.0을 0.1 단위로 받는다. 8탭이 한 번의 드래그가 됐다. UI 고도화(커스텀 그래픽 등)는 사용자 요청대로 다음으로 미루고 M3 기본 `Slider`로 심플하게 넣었다.
- **표시**: 0.1 단위가 되면 별 5개로는 4.3과 4.5가 똑같이 보인다 — 하필 구분이 필요한 곳이 목록이다. `DDRatingStars`를 `★ 4.3` 단일 별+숫자 형식으로 통일했다(대시보드 '종류별 평점 비교' 카드가 이미 쓰던 형식이라 앱 내 일관성도 올라감).
- **스키마**: `rating`이 처음부터 `REAL`이라 **마이그레이션이 필요 없었다**. 기존 0.5 단위 값은 0.1의 배수라 그대로 유효하고, 바뀐 건 `DrinkRating.kt`의 검증 규칙뿐이다. 에뮬레이터의 기존 0.5 데이터(4.5/3.5/5.0)가 그대로 표시되는 것과, 새로 저장한 4.3이 DB에 정확히 들어가는 것을 모두 확인했다.
- **부동소수 함정**: `(rating * 10).rem(1.0) == 0.0`으로 0.1 배수를 검사하면 안 된다 — `4.3 * 10`이 `43.000000000000007`이라 슬라이더가 만든 정상 값을 거부한다. `roundToStep()`으로 반올림한 값과 허용 오차로 비교하고, 저장 전에 소수점 1자리로 스냅한다.
- 미평가 상태는 `★ -`로 표시한다. 슬라이더 썸은 최소값 위치에 있을 수밖에 없어서, 숫자를 그대로 두면 손대지 않은 상태가 0.5를 고른 것처럼 읽힌다.
- 검증 중 배운 것: `adb input swipe`가 너무 빠르면(400ms) Compose `Slider`가 제스처를 놓친다. 값을 확실히 바꾸려면 트랙의 목표 지점을 눌렀다 떼는 편이 안정적이다.
