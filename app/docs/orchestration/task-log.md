...
> [!WARNING]
> **이 시점에서 코드는 명세와 어긋난다.** `TraitAnswer.Unsure`, 이진 판정
> (`ObserveTasteProfileUseCase`), 16유형(`TasteType`), `TasteThresholds`가 모두 구 명세 기준이다.
> 구현 전까지 앱은 **명세가 폐기한 방식으로 동작한다** — 특히 없는 취향을 지어내는 판정기가
> 그대로 살아 있다. 실기기/에뮬레이터 시연 시 이 점을 감안해야 한다.
