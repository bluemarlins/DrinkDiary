...
플래시 계열을 `gemini-3.5-flash-medium`/`gemini-3.6-flash-high` → **`gemini-3.7-flash-medium`/
`gemini-3.7-flash-high`**로 올렸다. `gemini-3.1-pro-high`와 `claude-sonnet-4-6`은 그대로다 —
새 세대가 구세대 **플래시**를 대체하는 것은 자연스럽지만, 다른 계열까지 대체한다고 보려면
그쪽 이력이 필요한데 아직 없다.
...
3.7 행의 "실사용 이력"은 비워 뒀다. 과거 기록(이 로그, 각 부서 문서의 출처 표기)은 **당시 실제로
쓴 모델을 그대로 남긴다** — 재현 정보를 소급 수정하면 재현이 불가능해진다.
