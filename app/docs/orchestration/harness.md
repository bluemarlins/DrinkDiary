...
`agy models`는 14개 모델을 노출하지만, 이 저장소에서 실제로 쓰는 것은 **아래 4개뿐이다**.
...
| `gemini-3.7-flash-high` | 창의·시각·멀티모달·리서치 | UI 다듬기, 카피/네이밍, 시장 조사, `generate_image` 에셋 생성 | ID 소통 확인만 됨(2026-08-16, conversation `465534bf`). 실작업 이력 없음 |
...
플래시 계열을 `gemini-3.5-flash-medium` / `gemini-3.6-flash-high`에서 **3.7 세대로 올렸다.**
역할 4칸은 그대로고 세대만 바뀌었다. 전환 전 `agy models`로 ID 실재를 확인했고
...
**이전 세대 기록은 아래에 남긴다.** 모델은 교체됐지만 거기서 얻은 교훈은 agy 자체의 성질이라
그대로 유효하다.
...
로스터 밖 모델(`*-low` 변형, `gemini-3.7-flash-high`를 뺀 3.6·3.5 계열 전부,
`gemini-3.1-pro-low`, `claude-opus-4-6-thinking`, `gpt-oss-120b-medium`)은 사용하지 않는다.
`-low` 변형은 품질 저하 대비 이득이 불분명하고 재시도 비용이 절감분보다 크며, 구세대 플래시는
3.7과 역할이 겹친다. 로스터를 바꾸려면 이 표를 먼저 고치고 근거를 남긴다.
...
```text
[코드 작업]  3.7-flash-medium → 3.1-pro-high → claude-sonnet-4-6 → Claude 직접
[창의/시각]  3.7-flash-high → Claude 직접
```

승급 경로에서 `gemini-3.1-pro-high`를 그대로 둔 이유: 3.7 플래시가 pro보다 낫다는 근거가 아직
**하나도 없다.** 새 세대가 구세대 플래시를 대체하는 것은 자연스럽지만, 다른 계열을 대체한다고
보려면 그쪽 이력이 필요하다. 3.7-flash-medium이 계산 로직에서 pro-high만큼 하는 것이 확인되면
그때 이 표를 3칸으로 줄인다.
