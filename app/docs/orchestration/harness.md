...
| `gemini-3.7-flash-high` | **추론이 필요한 전부** — 창의·시각·리서치 + 다단계 계산 로직 | UI 다듬기, 카피/네이밍, 시장 조사, 알고리즘 UseCase, `generate_image` 에셋 | 리서치 1건(2026-08-16, 수치 날조 0건이나 **UI 문구·개수 날조**) + **디자인 시스템 정렬 코드 6건 전부 성공**(2026-08-16, 아래 참조) |
...
#### `gemini-3.7-flash-high` 실사용 기록 — 디자인 시스템 정렬 6건 (2026-08-16)

명세 대조 감사에서 나온 수정 6건(T1~T6)을 전부 이 모델에 위임했다. **6건 모두 `status: SUCCESS`,
출력을 그대로 쓸 수 있는 품질이었다** — 지시한 값을 바꾸거나 명세를 재해석한 사례 0건.
Claude가 잡아낸 것은 넷뿐이고 전부 **누락**이지 오작성이 아니다: 토큰 값인 `4.dp`/`24.dp`를
리터럴로 남김(T3), 읽는 곳이 없는 `LocalDDWindowSize` 생성(T4), `NavigationRail` 선택 색 누락(T4).

**세 가지가 이번에 통한 이유이자, 다음에도 지켜야 할 것이다.**

1. **명세가 이미 확정돼 있었다.** 프롬프트에 hex 30개·브레이크포인트 3개·치환 규칙을 값으로
   적어 줬다. agy는 판단하지 않고 옮겼다 — `AGENTS.md`의 위임 조건 1번이 그대로 작동했다.
2. **편집 위임은 여전히 불가능하다.** `--dangerously-skip-permissions`가 이 세션에서도 분류기에
   차단됐다(2026-08-16 첫 시도와 동일). **agy가 코드를 출력하고 Claude가 적용**하는 방식으로
   전환했고, 결과적으로 검수가 강제돼서 위 네 건을 붙잡았다.
3. **긴 출력은 잘린다.** 파일 전문 + 표를 한 번에 요청하면 앞부분이 유실된다. `-c`로 같은 대화를
   이어 "그 파일만 다시 출력하라"로 회수했다(T4·T5·T6에서 각 1회).

**모델이 아니라 검증 범위가 문제였다.** 여섯 단계의 게이트(ktlint/lint/유닛테스트)가 전부
통과했는데도 T1이 넣은 크래시가 T6까지 살아남았다 — 에뮬레이터 확인을 매번 했지만 DB가 비어
있어 텍스트 필드가 있는 유일한 화면에 도달한 적이 없었다. 상세는
`../departments/designer/design-system-audit-2026-08.md` 5-1절.
