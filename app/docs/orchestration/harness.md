...
### 6-2. agy 위임 시 foundation 모델 선택 매트릭스

`agy models` 기준 사용 가능한 모델(2026-08-09 확인):
...
임의로 고르지 않고, 태스크가 아래 7개 축 중 어디에 해당하는지로 모델을 정한다. 여러 축에
걸치면 더 높은 tier 쪽을 따른다(과소 배정보다 과다 배정이 안전 — 실패 시 재시도 비용이 더 큼).

| 축 | 판단 질문 | 해당 시 추천 모델 | 근거 / 실사용 이력 |
| --- | --- | --- | --- |
| 추론 깊이 | 도메인 규칙이 다단계로 얽힌 계산/알고리즘 로직을 새로 설계·구현하는가? (예: 트렌드 계산, 만족도 가중치) | `gemini-3.1-pro-high` | P2-2(고급 인사이트 UseCase)에서 사용, 통과 확인 |
| 반복/보일러플레이트 | 기존 패턴을 그대로 복제하는 CRUD/Mapper/단순 UseCase인가? | `gemini-3.5-flash-medium` (실패 시 `-low`는 쓰지 않고 상위로 승급) | P2-5(CSV export UseCase)에서 사용, 통과 확인 |
| 시각/주관적 판단 | 정답이 하나로 정해지지 않는 UI 배치·모션·카피 다듬기인가? | `gemini-3.6-flash-high` | P2-3(인사이트 UI), P1-7(아이콘 컨셉) |
| 순수 텍스트 산출 (코드 아님) | 리서치 요약/브랜딩 카피/네이밍처럼 사람이 후처리로 필터링할 대량 텍스트인가? | `gemini-3.5-flash-medium`(정형 보고서) 또는 `gemini-3.6-flash-high`(창의성 우선, 예: 네이밍 브레인스토밍) | P0-4(벤치마킹), P1-1(네이밍), P1-4(스토어 설명) |
| 멀티모달 | 실제 이미지(PNG 등) 생성이 필요한가? | `gemini-3.6-flash-high` (`generate_image` 툴 보유 — Claude 세션엔 없음) | P1-7 |
| Claude 코드 정확도가 필요하지만 오케스트레이터(Claude Code 세션) 직접 처리는 피하고 싶은 경계 케이스 | 아키텍처 규칙 준수가 중요한데 서브트리로 범위를 좁힐 수 있어 위임 가능한가? | `claude-sonnet-4-6`(agy 경유) | 아직 실사용 이력 없음 — 첫 사용 시 결과를 이 표에 반영 |
| 2차 의견/대안 검토 | Claude(오케스트레이터)의 아키텍처 판단을 다른 모델로 교차 검증하고 싶은가? | `claude-opus-4-6-thinking`(agy 경유) | 상시 사용 아님, 필요 시에만. 코드 편집이 아니라 리뷰/의견 청취 용도로 우선 활용 |

`gpt-oss-120b-medium`은 위 어떤 축에도 아직 배정하지 않는다(실사용 이력 없음). 다른 모델 결과가
반복적으로 기준 미달일 때 비교군으로 시험해보고, 유용성이 확인되면 이 표에 축을 추가한다.

**동적 승급 규칙**: 선택한 모델로 만든 결과물이 Definition of Done 게이트(ktlint/lint/유닛테스트)를
2회 연속 통과하지 못하면, 같은 모델로 재시도하지 않고 한 단계 위 tier로 승급해 재시도한다
(`flash-low → flash-medium → flash-high → pro-high`, 또는 Gemini 계열 전반이 반복 실패하면
`claude-sonnet-4-6`으로 계열 자체를 전환). 승급 이력은 `task-log.md` 비고에 남긴다.

**Claude 직접 처리로 되돌리는 경우**: 승급이 `pro-high`/`claude-sonnet-4-6`까지 갔는데도 게이트를
통과하지 못하면, 더 이상 agy에 위임하지 않고 Claude(오케스트레이터)가 직접 작성한다 — 태스크
자체가 6절 원칙표상 "agy 위임" 범주로 잘못 분류됐을 가능성이 크다는 신호로 본다.
