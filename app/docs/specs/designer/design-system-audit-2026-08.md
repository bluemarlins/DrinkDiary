...
> [!NOTE]
> **확정 명세 (2026-08-17 승격).** 점검: Claude(코드 대조). agy 위임 없음 —
> 우리 명세와 우리 코드를 맞대는 일이라 검증이 곧 작업 자체다.
>
> **이 문서는 감사 기록이지 규범이 아니다.** 색·타이포·컴포넌트의 **정답은 `design-system.md`**이고,
> 여기 적힌 값은 그 정답에 도달하기까지의 근거다. §2가 열거하는 결함은 **2026-08-16~17에 전부
> 수정되어 지금 코드에는 없다**(T1~T9, 커밋 `c4604f9`~`955578c`). 규범으로 읽으면 정반대의
> 결론이 나오므로, 이 문서를 근거로 코드를 고치기 전에 반드시 `design-system.md`를 먼저 본다.
>
> 승격한 이유는 두 가지다. §3의 명세 정정 결정(`Malt` `#8A5A1B`, §2 `20dp`)이 사용자 확정을
> 받았고, §5의 교훈 — 특히 **5-1 "게이트가 통과해도 앱은 죽을 수 있다"** — 이 앞으로의 작업에
> 계속 적용되는 규칙이기 때문이다.
>
...
1. **`Malt` / `MaltContainer` 재설계** (2-4 ⑦). 지금 값으로는 코드를 명세에 맞춰도 AA에 못 미친다.
   → **결정: `Malt` Light를 `#9C6722` → `#8A5A1B`로 한 단계만 어둡게 한다** *(2026-08-17 사용자 확정)*
...
- ~~**모션 (2-5)**~~ — **T8에서 Depth In/Out과 탭 전환까지 완료.** 깊이가 다른 이동은 Z축
  ...
  **`Selection Mode 일괄 작업 바`는 T8 시점에 만들지 않았다** — 명세 1절 3번에만 있고
  `../planner/prd.md`·`../planner/mvp-scope.md` 어디에도 대응 기능이 없었기 때문이다(없는 제품 동작을 발명하지 않는다).

  → **2026-08-17 해소.** 사용자가 확정했다: **MVP에 넣고, 되돌리기는 확인 팝업 한 번이 전부**다.
  `../planner/prd.md` F1-2를 먼저 신설하고(커밋 `2b3af9c`) 그 위에 구현했다(T11·T12).
  근거 문서(`../../departments/designer/design-system-ux-research.md` 4.2절 3번)에는 원래 인터랙션까지 정의돼 있었다 —
...
- ~~**화면별 적응형 레이아웃 (§4 마지막 열)**~~ — **T9에서 완료.** T4가 내비게이션 구조와 마진까지
  ...
  - **기록 마법사(F2)·편집·설정은 1단으로 남긴다.** 2단을 강제하면 기본 입력 경로가 무거워져
    `../planner/prd.md` F2의 탭 예산과 충돌한다. §4의 문면보다 F2가 우선한다고 판단했다.
...
각 단계의 Definition of Done은 `../../orchestration/harness.md` §4 그대로다. 추가로 **`DesignTokenTest`(Claude 작성)**를
