...
| **Expanded** | `≥ 840dp` (태블릿/DeX) | `32dp` | `NavigationRail` (좌측, 아이콘) | List-Detail / Supporting Pane (최대 콘텐츠폭 720dp 제한) |

---

## 4-1. 플로팅 크롬 (Floating Chrome) *(2026-08-19 신설)*

Compact 최상위 화면의 상·하단 바는 **콘텐츠 위에 떠 있고, 콘텐츠는 그 뒤로 흐른다.**
기조는 하나다 — **화면에 콘텐츠를 최대한 많이 보여 준다.**

| 요소 | 규격 |
| :--- | :--- |
| 좌우 여백 | `LocalDDScreenMargin` (Compact 16dp) |
| 모양 | `ShapeLarge` 18dp |
| 배경 | `hazeChild` 블러 + `Surface` 틴트 (하단 0.42, **상단 0.58**) |
| 테두리 | 1dp `outlineVariant` |
| 높이 | 상단 `DDTopAppBarHeight` 72dp, 하단 `DDBottomNavigationBarHeight` 80dp |

**상단 틴트가 하단보다 진하다.** 상단에는 아이콘이 아니라 **타이틀 텍스트**가 얹히고, 스크롤이
시작되는 쪽이라 사진의 밝은 윗부분이 지나갈 확률이 높다. 텍스트는 대비가 조금만 흔들려도
아이콘보다 먼저 안 읽힌다.

### 4-1-1. 상단 바는 항상 떠 있지 않다 — 겹칠 때만 뜬다

- **정지 상태**: 크롬 없는 맨 타이틀(도킹 앱바와 같은 자리, 같은 모양).
- **콘텐츠가 앱바 영역과 겹치기 시작하면**: 알약이 **맺힌다**. 블러 반경·틴트·테두리 알파·
  안쪽 여백을 `0 → 1` 하나의 값으로 함께 보간하므로 켜지는 것이 아니라 자라난다.
- **알약 폭은 화면 폭이 아니라 `wrapContentWidth`, 즉 글자 폭이다.** 전폭 알약은 도킹 바의
  모서리만 깎아 놓은 것과 다르지 않고, 가리는 면적이 그대로다.
- 겹침 판정은 스캐폴드의 `NestedScrollConnection`이 자식 스크롤 델타를 주워서 한다 —
  화면마다 스크롤 컨테이너가 달라도(`Column` / `LazyColumn`) 화면 코드를 고치지 않는다.

### 4-1-2. 페이딩 엣지

콘텐츠는 화면 가장자리에서 **알파로 사라진다**(위: 상태바 높이, 아래: 24dp).
잘린 글자 반 줄은 "여기가 끝"이 아니라 "덜 그려졌다"로 읽힌다.

**페이드 범위를 바에 맞추지 않는다.** 바 뒤까지 지우면 블러가 비출 것이 없어져 플로팅이
죽는다 — 화면 가장자리에만 건다.

### 4-1-3. 화면이 지켜야 할 것

**인셋을 바깥 `Modifier.padding`으로 먹지 않는다.** 스크롤 컨테이너의 `contentPadding`으로
비운다. 바깥에 걸면 콘텐츠가 바 뒤로 흐르지 않아 블러가 비출 것이 없어지고, 플로팅이 아니라
그냥 떠 있는 불투명 막대가 된다.

> [!NOTE]
> **Expanded의 영구 드로어를 레일로 바꿨다** *(2026-08-20 사용자 확정)*. M3 `PermanentDrawerSheet`의
> 기본 폭이 360dp라, 876dp 가로 화면에서 **왼쪽이 41%를 먹고 본문에 516dp만 남았다** — 넓히려고
> 가로로 돌린 화면을 목적지 이름 넉 줄로 되돌려주는 꼴이었다.
>
> **레일 폭은 80dp 고정이며 화면 폭에 비례시키지 않는다.** 아이콘 하나의 터치 타깃은 화면이
> 넓어진다고 커질 이유가 없고, 비례로 잡으면 넓은 화면일수록 더 낭비한다. 폭에 반응해야 하는
> 것은 내비게이션이 아니라 **본문**이고, 그건 이미 720dp 상한이 맡고 있다.
>
> Medium과 Expanded가 같은 레일을 쓰므로 둘의 차이는 이제 **마진(24 / 32dp)과 본문 상한**뿐이다.
