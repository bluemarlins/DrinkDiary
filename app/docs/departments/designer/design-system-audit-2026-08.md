...
- ~~**모션 (2-5)**~~ — **T8에서 Depth In/Out과 탭 전환까지 완료.** 깊이가 다른 이동은 Z축
  ...
  **`Selection Mode 일괄 작업 바`는 만들지 않았다.** 명세 1절 3번에만 있고 `prd.md`·`mvp-scope.md`
  어디에도 대응 기능이 없다. 만들면 없는 제품 동작을 발명하는 것이라(`AGENTS.md`) 남겨 둔다 —
  **명세 §1-3과 PRD의 불일치이므로 기획이 정해야 한다.**
- **화면별 적응형 레이아웃 (§4 마지막 열)** — T4는 §4의 **내비게이션 구조와 마진까지만** 맞췄다.
  Medium의 "2-Column 분할(좌 40% : 우 60%)"과 Expanded의 "List-Detail / Supporting Pane"은
  화면마다 다른 정보 구조를 요구하므로 컴포넌트 정렬이 아니라 화면 재설계다. 그대로 남는다 —
  **§4의 절반은 아직 구현되지 않았다.**
