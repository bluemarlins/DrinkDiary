...
Claude acts as orchestrator for the Play Store launch/monetization initiative (`app/docs/service-launch-roadmap.md`): code changes, design-system structuring, and merging all outputs stay with Claude; research, image generation, and git commit/push are delegated to the `agy` CLI (a multi-model CLI pointed at Gemini models — not the `gemini` npm package). Use the `agy-research` skill to delegate research/image-gen and the `agy-commit` skill to delegate commit/push — both encode the exact invocation flags and gotchas learned the hard way (model selection, absolute paths, trailing-punctuation-breaks-the-command, cwd not inherited). Don't hand-roll `agy` invocations from scratch; read the skill first.

**Verification-first**: don't declare UI/UX work done on inspection alone. Use the `verify-build` skill (build → unit tests → install → launch → screenshot) after code changes, especially UI changes. For design/UX polish work specifically, use the `ui-quality-reviewer` subagent to get an independent critique from `agy` before considering a screen finished — see "UI 품질 루프" in `service-launch-roadmap.md`.

**Explore → Plan → Code**: changes touching a whole screen or multiple files (most UX-elevation work) should go through plan mode first. Skip planning only when the diff is describable in one sentence.
